...
CzBALLTracker::CzBALLTracker(std::string strWalletFile)
{
...
CoinWitnessData* CzBALLTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBALLTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
{
    bool iszBALLWalletInitialized = (NULL != zBALLWallet);
    CMintMeta meta;
    ...
    if (! iszBALLWalletInitialized)
        zBALLWallet = new CzBALLWallet(strWalletFile);
...
std::set<CMintMeta> CzBALLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBALLWallet* zBALLWallet = new CzBALLWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zBALLWallet);
        }
        delete zBALLWallet;
